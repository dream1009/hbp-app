  ...
  <div class="approval-history">
    <van-steps :active="active" direction="vertical" active-color="#f55353">
      <van-step v-for="(item, index) in opinions" :key="index">
        <h3>
          ...
          <span v-if="item.batch">
            <span v-if="item.qualifiedExecutor&&item.status == 'pending'">
              {{ getName(item.qualifiedExecutor) }}
            </span>
          ...
          <van-tag :class="getStatusType(item,index)" plain>
            {{ item.statusName }}
          </van-tag>
          </a>
        </h3>

        <!--<p v-if="item.opinion" class="opinion">
          {{ item.opinion }}
        </p>-->
        <card-list
          :avatar="false"
        >
          <!--标题-->
          <div v-for="(field,i) in formFields" :key="'field'+i" slot="title" class="van-card-list__desc">
            <span class="van-card-list__desc__title">{{ field['label'] }}:</span>
            <field-formatter
              slot="title"
              class="titles"
              :label-key="field['name']"
              :data="getObject(item.approvalForm.formValue)"
...
<!--          到达: {{ item.createTime|formatDate('yyyy-MM-dd HH:mm:ss') }}<br>-->
          审批时间: {{ item.completeTime|formatDate('yyyy-MM-dd HH:mm:ss') }}
        </p>
      </van-step>
    </van-steps>
  </div>
  ...
  methods: {
    ...
    loadData() {
      getOpinions({
        ...
        if (variables && variables.procHisFormDef) {
          const procHisFormDef = JSON.parse(variables.procHisFormDef)
          if (procHisFormDef && procHisFormDef.fields) {
              this.formFields = procHisFormDef.fields
          }
        }
        this.setActive()
      })
