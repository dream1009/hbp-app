  ...
  <van-popup slot="fr-ext" v-model="popup" class="lc-fullscreen-popup">
    <div class="lc-fixed-top">
      ...
      <van-cell-group>
        ...
        />
        <van-lc-picker
          v-if="data.msgSources==='热线系统'"
          v-model="toCallCenter"
          label="退单级别"
          :options="toCallCenterOptions"
          clearable
        />
        <van-field
          v-model="remark"
      ...
      <van-cell-group v-if="actionName==='oppose'">
        <van-lc-checker
...
export default {
  ...
  computed: {
    actions() {
      ...
      return [{
        'name': this.$t('common.button.chargeback'),
        'buttonType': 'primary',
  ...
  methods: {
    ...
    onSave(data) {
      ...
      data.data['toCallCenter'] = this.toCallCenter
      data.data['workorderState'] = '申请退单'
      // data.data['t_workorder_maintain'] = [{ 'arriveTime': fecha.format(new Date(), '') }]
      data.data['t_workorder_back'] = [{ 'backUser': this.user.userId, 'backUserId': this.user.userId, 'workorderid': data.data.workorderid,
        'backTime': fecha.format(new Date(), ''), 'backRemark': this.remark, 'baseTaskid': data.data.baseTaskid }]
